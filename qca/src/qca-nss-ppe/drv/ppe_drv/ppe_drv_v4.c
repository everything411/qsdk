...
/*
 * ppe_drv_v4_conn_flow_igmac_add()
 *	Ingress mac address add API
 */
static bool ppe_drv_v4_conn_flow_igmac_add(struct ppe_drv_v4_conn_flow *pcf)
{
	...
	ppe_drv_v4_conn_flow_flags_set(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_IGMAC_VALID);
	return true;
}

/*
 * ppe_drv_v4_conn_flow_igmac_del()
 *	Ingress mac address delete API.
 */
static bool ppe_drv_v4_conn_flow_igmac_del(struct ppe_drv_v4_conn_flow *pcf)
{
	struct ppe_drv_iface *in_l3_if;
	...
	ppe_drv_v4_conn_flow_flags_clear(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_IGMAC_VALID);
	return true;
}
#endif
...
bool ppe_drv_v4_if_walk(struct ppe_drv_v4_conn_flow *pcf, struct ppe_drv_top_if_rule *top_if, ppe_drv_iface_t tx_if, ppe_drv_iface_t rx_if)
{
	...
	if (ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLOW_FLAG_BRIDGE_FLOW)) {
		...
		if (top_iface_parent)
			ppe_drv_v4_conn_flow_eg_top_vsi_set(pcf, ppe_drv_iface_vsi_get(top_iface_parent));

		ppe_drv_info("%p: Return for v4 bridge flow\n", p);
		return true;
	}
...
static bool ppe_drv_v4_flow_del(struct ppe_drv_v4_conn_flow *pcf)
{
	 ...
	 */
	if (ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_IGMAC_VALID)) {
		ppe_drv_v4_conn_flow_igmac_del(pcf);
	}
#endif
	/*
...
static struct ppe_drv_flow *ppe_drv_v4_flow_add(struct ppe_drv_v4_conn_flow *pcf)
{
	 ...
	 */
	if (!(ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLOW_FLAG_BRIDGE_FLOW) ||
	     ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLAG_FLOW_PPE_ASSIST))) {
	...
	if (ppe_drv_v4_conn_flow_flags_check(pcf, PPE_DRV_V4_CONN_FLOW_FLAG_BRIDGE_FLOW)) {
		...
		vlan_cnt = ppe_drv_v4_conn_flow_egress_vlan_cnt_get(pcf);
		if (vlan_cnt) {
			pp_tx = ppe_drv_v4_conn_flow_tx_port_get(pcf);
			if (!pp_tx) {
			...
			eg_top_vsi = ppe_drv_v4_conn_flow_eg_top_vsi_get(pcf);
			if (!eg_top_vsi) {
			...
			if (!(eg_top_vsi->is_fdb_learn_enabled && pp_tx->is_fdb_learn_enabled && pp_rx->is_fdb_learn_enabled)) {
				nh = ppe_drv_nexthop_v4_bridge_flow_get_and_ref(pcf);
				if (!nh) {
	 ...
	 */
	if (!ppe_drv_v4_conn_flow_igmac_add(pcf)) {
...
/*
 * ppe_drv_v4_port_offload_enabled()
 *	check if the offload is enabled for the rule's port or not
 */
static bool ppe_drv_v4_port_offload_enabled(struct ppe_drv_v4_rule_create *create)
{
	struct ppe_drv *p = &ppe_drv_gbl;
...
ppe_drv_ret_t ppe_drv_v4_create(struct ppe_drv_v4_rule_create *create)
{
	 ...
	 */
	if (!ppe_drv_v4_port_offload_enabled(create)) {
		spin_unlock_bh(&p->lock);
		ppe_drv_stats_inc(&comm_stats->v4_create_offload_disabled);
